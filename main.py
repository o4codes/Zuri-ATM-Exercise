...
def register_page():
    while True:
        ...
        if response:
            ...
            print("\nAccount Number: "+get_account(name)['account_number'])
        ...
        if response == "success":
            ...
            try:
                ...
                if selected_option == 1:
                    ...
                    is_proceed = next_transaction_dialog()
                    if is_proceed == 1:
                        break
                    ...
                    try:
                        ...
                        balance_left = withdraw(get_account(
                            name)['balance'], amount_withdraw)
                    except AssertionError:
                        print(
                            "amount to withdraw cannot be greater than current balance")
                    except ValueError:
                    ...
                    try:
                        ...
                        current_balance = deposit(get_account(
                            name)['balance'], deposit_amount)
                    ...
                    print("\nAccount Number: " +
                          get_account(name)['account_number'])
                    is_proceed = next_transaction_dialog()
                    if is_proceed == 1:
                        break
...
# contains next transaction console prompt
def next_transaction_dialog():
    try:
        next_reply = int(input(
            "\nEnter 0 if you wish to perform another transaction \nEnter 1 if you wish to log-out\n"))
    except ValueError:
        print("\nInvalid response, you will be logged out")
        home_page()
        return 1
    else:
        if next_reply == 1:
